...
int addMember(list **listHead, void *data)
{
    ...
    list *curr = (list *)calloc(sizeof(list), 1); //list * localHead = NULL;
...
/*  * The compare function (compare_func) is user implemented. 

 * It takes in two parameters:  * 1. void * of value 1  * 2. void * of value 2  *  * The function return 1 on equals, 0 otherwise  *  * find_member traverses the linked list starting at the head  * and finds the first element of interest by using compare_func.  *  * It takes the address of the value its trying to find a a function  * pointer of type compare_func. It feeds its first parameter to  * as a first argument to compare_func and the data field in the  * linked list structure definition as the second argument to compare_func.  *  * On success find_member returns a pointer to the list_entry of interest.  * On failure it returns NULL.  */
...
list *find_member(list **listHead, void *value, compare_func cmp_func)
{
    ...
    //precautionary measure list * curr = (*listHead)->listRoot->head;
...
int removeMember(list **listHead, void *value, compare_func cmp_func)
{
    ...
    if (listHead == NULL || (*listHead) == NULL)
    {
        perror("Removing member has failed.\n"
               "list head is NULL\n");
        return 0;
    } //precautionary measure list * localHead = (*listHead)->listRoot->head;
...
int removeHead(list **listHead)
{
    ...
    list *oldhead = curr;
    if (newHead == NULL)
    { //head is the only element free(curr->listRoot); free(curr); (*listHead) = NULL; return 1; }

        newHead->prev = NULL;
        (*listHead) = newHead;
        newHead->listRoot->head = newHead; //free(oldhead->data); free(oldhead);
    ...
    int removeTail(list * *listHead)
    {
        ...
        (*listHead) = head; //free(oldTail->data); free(oldTail);
        return 1;
    }

    /*  * doFunc is a user defined function to perform on  * every data member of the linked list structure  * as the memory allocated to the linked list structure  * is freed.  */ void destroyList(list * *listHead, doFunction func)
    {
        ...
        do
        {
            ...
            //free(curr->data); free(curr); }while((curr = curr->next) != NULL);
